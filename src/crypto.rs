...
impl EncryptionKey {
    ...
    /// Raw key bytes (for tests or direct cipher construction).
...
/// Input: list of base64-encoded encrypted values.
/// Returns the re-encrypted values in the same order.
pub fn rotate_key(
...
mod tests {
    ...
    fn key_rotation() {
        ...
        // Old key can't decrypt rotated values
        assert!(decrypt(&old_key, &rotated[0]).is_err());
        // New key can decrypt them
    ...
    fn invalid_ciphertext_errors() {
        ...
        assert!(decrypt(&key, "dG9vc2hvcnQ=").is_err()); // "tooshort" in base64
    }
